...
Details: Group Creation: Adds a new group called cloudadmins. User Accounts: Creates three user accounts (user100, user200, and user300). Group Membership: Adds these users to the cloudadmins and wheel groups. SSH Keys: Generates SSH keys for each user without a passphrase. Key Management: Downloads the private key for user100 from a specific virtual machine to your local computer.
...
Details: Installation: Installs Apache web server software. File Management: Creates and uploads HTML files (vm1.html, vm2.html, and vm3.html) with each file containing the FQDN of the respective node. Permissions: Copies these files to the /var/www/html directory and sets permissions to 0444 (read-only). Service Management: Starts the Apache web server and ensures it starts automatically on system reboots.
